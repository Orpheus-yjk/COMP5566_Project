        ...
        for call in list_of_calls[index]:
            if isReal(call["input_size"]) and call["input_size"] > 0 and is_expr(call["recipient"]) and any([True for var in get_vars(call["recipient"]) if str(var) == "Ia"]):
                if call["input_offset"] in call["memory"]:
                    function_signature = call["memory"][call["input_offset"]]/26959946667150639794667015087019630673637144422540572481103610249216
...
def detect_honeypots(FILENAME):
    ...
    path = "./result/{}".format(FILENAME)
    if not os.path.exists(path):
        os.makedirs(path)
    resultwritter = open(path+"/result.txt","w", encoding="utf-8")
    print("money_flow_found", money_flow_found, file=resultwritter)
    print("balance_disorder_found", balance_disorder_found, file=resultwritter)
    print("hidden_transfer_found", hidden_transfer_found, file=resultwritter)
    print("inheritance_disorder_found", inheritance_disorder_found, file=resultwritter)
    print("uninitialised_struct_found", uninitialised_struct_found, file=resultwritter)
    print("type_deduction_overflow_found", type_deduction_overflow_found, file=resultwritter)
    print("skip_empty_string_literal_found", skip_empty_string_literal_found, file=resultwritter)
    print("hidden_state_update_found", hidden_state_update_found, file=resultwritter)
    print("straw_man_contract_found", straw_man_contract_found, file=resultwritter)
    resultwritter.close()
...
def main(FILENAME, contract, contract_sol, _source_map = None):
    global c_name
    global c_name_sol
    global source_map
    global start_time
